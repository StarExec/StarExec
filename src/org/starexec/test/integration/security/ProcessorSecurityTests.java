...
public class ProcessorSecurityTests extends TestSequence {
	...
	private void canUserEditProcessorBadDescTest() {
		Assert.assertFalse(ProcessorSecurity.canUserEditProcessor(p.getId(), admin.getId(), "test", TestUtil.getRandomAlphaString(DB.SPACE_DESC_LEN+1)).isSuccess());
		Assert.assertFalse(ProcessorSecurity.canUserEditProcessor(p.getId(), owner.getId(), "test", TestUtil.getRandomAlphaString(DB.SPACE_DESC_LEN+1)).isSuccess());
		Assert.assertFalse(ProcessorSecurity.canUserEditProcessor(p.getId(), otherUser.getId(), "test", TestUtil.getRandomAlphaString(DB.SPACE_DESC_LEN+1)).isSuccess());
		Assert.assertFalse(ProcessorSecurity.canUserEditProcessor(p.getId(), notInCommunity.getId(), "test", TestUtil.getRandomAlphaString(DB.SPACE_DESC_LEN+1)).isSuccess());
	}
