...
public class SolverSecurity {
	...
	/**
	 * Checks whether the user can see the build log for the given solver. They must own the solver or be an admin
	 *
	 * @param solverId
	 * @param userId
	...
	/**
	 * Checks to see whether the given user can upload a solver with a starexec_build script in the given space. Only
	 * leaders of communities may do this, and they can only do it in their own community
	 *
	 * @param userId
	 * @param spaceId
	 * @return
	...
	public static ValidatorStatusCode canUserRunStarexecBuild(int userId, int spaceId) {
		...
		if (p == null || !p.isLeader()) {
			return new ValidatorStatusCode(
					false,
					"You do not have permission to use a starexec_build script. Only community leaders may use build " +
							"scripts."
			);
		}
		return new ValidatorStatusCode(true);
	...
	public static ValidatorStatusCode canUserGetAnonymousLink(int solverId, int userId) {
		if (userOwnsSolverOrIsAdmin(Solvers.get(solverId), userId)) {
			...
			return new ValidatorStatusCode(
					false, "You do not have permission to get an anonymous link for this solver.");
		}
	}
	 ...
	 * @return new ValidatorStatusCode(true) if the operation is allowed or a status code from ValidatorStatusCodes if
	 * not
	 */
	...
	public static ValidatorStatusCode canUserUpdateSolver(
			int solverId, String name, String description, boolean isDownloadable, int userId
	) {
		// Ensure the parameters are valid
		if (!Validator.isValidSolverName(name)) {
			return new ValidatorStatusCode(
					false,
					"The new name is not in the correct format. Please see the help pages to see the correct format"
			);
		}
		...
		if (!Validator.isValidPrimDescription(description)) {
			return new ValidatorStatusCode(
					false,
					"The new description is not in the correct format. Please see the help pages to see the correct " +
							"format"
			);
		}
	 ...
	 */
	public static ValidatorStatusCode canUserUpdateConfiguration(
			int configId, int userId, String name, String description
	) {
		// Ensure the parameters are valid
		...
		if (!userOwnsSolverOrIsAdmin(solver, userId)) {
			return new ValidatorStatusCode(
					false, "You do not have permission to update any configurations associated with this solver");
		}
		return new ValidatorStatusCode(true);
	}
	...
	public static ValidatorStatusCode canUserRecycleOrphanedSolvers(int userIdToDelete, int userIdMakingRequest) {
		...
		if (userIdToDelete != userIdMakingRequest && !GeneralSecurity.hasAdminWritePrivileges(userIdMakingRequest)) {
			return new ValidatorStatusCode(
					false, "You do not have permission to recycle solvers belonging to another user");
		}
	...
	public static ValidatorStatusCode canUserDeleteConfiguration(int configId, int userId) {
		...
		if (solver == null) {
			return new ValidatorStatusCode(
					false, "The solver associated with the given configuration could not be found");
		}
		if (!userOwnsSolverOrIsAdmin(solver, userId)) {
			return new ValidatorStatusCode(
					false, "You do not have permission to delete configurations attached to this solver");
		}
		return new ValidatorStatusCode(true);
	}
