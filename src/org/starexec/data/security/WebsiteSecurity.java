...
public class WebsiteSecurity {
	...
	public static ValidatorStatusCode canUserAddWebsite(int primId, String type, int userId, String name, String URL) {
		if (!Validator.isValidWebsiteName(name)) {
			return new ValidatorStatusCode(
					false,
					"The website name is not formatted correctly. Please refer to the help pages to see the correct " +
							"format"
			);
		}
		...
		if (!Validator.isValidWebsite(URL)) {
			return new ValidatorStatusCode(
					false,
					"The website url is not formatted correctly. Please refer to the help pages to see the correct " +
							"format"
			);
		}
		switch (type) {
	...
	private static ValidatorStatusCode canDeleteSpaceWebsite(Website site, int userId) {
		...
		if (p == null || !p.isLeader()) {
			return new ValidatorStatusCode(
					false, "You do not have permission to delete websites associated with this space");
		}
