...
public class BenchmarkSecurity {
	...
	public static ValidatorStatusCode canUserGetAnonymousLink(int benchmarkId, int userId) {
		...
		if (!userOwnsBenchOrIsAdmin(bench, userId)) {
			return new ValidatorStatusCode(
					false, "You do not have permission to generate an anonymous link for this benchmark.");
		} else {
			return new ValidatorStatusCode(true);
		}
	}
	...
	/**
	 * Checks to see whether the given user is allowed to see the contents of the given benchmark
	 *
	 ...
	 * @param userId The ID of the user making the request
	 * @return 0 if allowed, or a status code from ValidatorStatusCodes if not allowed. If the user doesn't have the
	 * required permissions for even 1 benchmark, a status code will be returned
	 */
	...
	public static ValidatorStatusCode canUserEditBenchmark(
			int benchId, String name, String desc, int typeId, int userId
	) {
		...
		if (!Validator.isValidBenchName(name)) {
			...
			return new ValidatorStatusCode(
					false,
					"The new name is not valid. Please refer to the help pages to find format for benchmark names"
			);
		}
		...
		if (!Validator.isValidPrimDescription(desc)) {
			return new ValidatorStatusCode(
					false,
					"The new description is not valid. Please refer to the help pages to find format for benchmark " +
							"descriptions"
			);
		}
	...
	public static ValidatorStatusCode canUserRecycleOrphanedBenchmarks(int userIdToDelete, int userIdMakingRequest) {
		...
		if (userIdToDelete != userIdMakingRequest && !GeneralSecurity.hasAdminWritePrivileges(userIdMakingRequest)) {
			return new ValidatorStatusCode(
					false, "You do not have permission to recycle benchmarks belonging to another user");
		}
