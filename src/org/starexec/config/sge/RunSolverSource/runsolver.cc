...
private:
  ...
  public:
	...
	RunSolver::instance->stopSolver("Mem limit exceeded: sending SIGQUIT then SIGKILL",SIGQUIT);
      }
    }
  private:
  ...
  static void watcherSigHandler(int signum, siginfo_t *siginfo, void *ucontext)
  {
    ...
    {
      instance->stopSolver("Received SIGTERM or SIGINT or SIGQUIT, killing child",SIGQUIT);
      return;
    }
  }
  ...
  static void *timerThread(void *)
  {
	...
	-instance->starttv.tv_sec-instance->starttv.tv_usec/1E6;
      ...
      if (count==10)
      {
	// identify new children every 10 loops, = 1s
...
//      procTree->dumpCPUTimeAndVSize(cout,currentCPUTime,currentVSize);
    ...
    if (limitCPUTime && currentCPUTime+lostCPUTime>=limitCPUTime)
      stopSolver("Maximum CPU time exceeded: sending SIGXCPU then SIGKILL",SIGXCPU);

    if (limitWallClockTime && elapsed>=limitWallClockTime)
      stopSolver("Maximum wall clock time exceeded: sending SIGALRM then SIGKILL",SIGALRM);

    if (limitVSize && currentVSize>=limitVSize)
      stopSolver("Maximum VSize exceeded: sending SIGQUIT then SIGKILL",SIGQUIT);
...
public:
    ...
    */
	...
	if(varOutputFilename)
	{
	  ...
	  var << "# CPUUSAGE: CPUTIME/WCTIME in percent\n"
	      << "CPUUSAGE=" << 100*virtualtime/(float)((realtime2!=0)?realtime2:1)
	      << endl;
   ...
   */
  void stopSolver(const char *msg,int signal)
  {
    int err;
  ...
  void sendSIG(int signal)
  {
    pid_t pidToSignal=childpid;

#ifdef SENDSIGNALBOTTOMUP
    cout << "\nSending " << getSignalName(signal) << " to process tree (bottom up)" << endl;
    procTree->sendSignalBottomUp(signal);
#else
    // signal the whole group
    pidToSignal=-childpid;

    cout << "\nSending " << getSignalName(signal) << " to " << pidToSignal << endl;
    kill(pidToSignal,signal);
#endif
  }
