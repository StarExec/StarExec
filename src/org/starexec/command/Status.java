...
/**
 * This class defines the codes that are used by StarexecCommand. All codes are
 * integers-- all codes that indicate errors are less than 0. 0 is the code used
 * for success.
 *
 * @author Eric
 */
public class Status {
	...
	static {
		...
		messages.put(ERROR_MISSING_PARAM,
				"Command is missing a required parameter-- please consult the StarexecCommand reference");
		messages.put(ERROR_FILE_NOT_FOUND, "The specified file could not be found");
		messages.put(ERROR_ARCHIVE_NOT_FOUND,
				"You do not have permission to download the requested archive, or the archive does not exist-- please ensure the given ID is correct");
		messages.put(ERROR_FILE_EXISTS, "The specified filepath already exists-- use the flag \"ow\" to overwrite.");
		messages.put(ERROR_BAD_PARENT_SPACE,
				"You do not have permission to add subspaces to the given parent space, or the parent space does not exist");
		...
		messages.put(ERROR_CONNECTION_EXISTS,
				"You must log out of the existing session before you can start a new one");
		messages.put(ERROR_BAD_URL,
				"The given URL does not point to a valid Starexec instance. Ensure that you are using the correct protocol (http vs https) and that the address ends with a /");
		messages.put(ERROR_BAD_INSTITUTION, "The institution given has invalid characters or is too long");
		messages.put(ERROR_PERMISSION_DENIED,
				"You do not have permission to view the contents of the given space, or the space does not exist");
		messages.put(ERROR_COMMAND_FILE_TERMINATING,
				"An error was encountered: the file of commands may not have been completed");
		...
		messages.put(ERROR_BAD_TRAVERSAL_TYPE, "The traversal must be either depth-first (" + C.ARG_DEPTHFIRST
				+ ") or round-robin (" + C.ARG_ROUNDROBIN + ")");
