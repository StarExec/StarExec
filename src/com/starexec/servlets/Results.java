...
public class Results extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(Util.paramExists(P.JOB_ID, request)) {
			// If it has a job id parameter, handle the request as a job
			this.handleJob(request, response);
		} else if (Util.paramExists(P.PAIR_ID, request)) {
			// Else if it has a pair id parameter, handle the request as a pair
			this.handlePair(request, response);
		}							
	}
	...
	private void handleJob(HttpServletRequest request, HttpServletResponse response){
		...
		// Get the status (may be empty/null)
		String status = request.getParameter(P.JOB_STATUS);
		
		// Get the node the ran on (may be empty/null)
		String node = request.getParameter(P.JOB_NODE);
			
		Databases.next().updateJobStatus(jobId, status, node);
	...
	private void handlePair(HttpServletRequest request, HttpServletResponse response){
		...
		// Default start/end time in case they aren't supplied by the caller
		Long startTime = -1L;
		Long endTime = -1L;
		...
		Databases.next().updatePairResult(pairId, status, result, node, startTime, endTime);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Posts not supported for Results servlet
	}
}
