...
function makeTableDraggable(table, dragStart, dragClone) {
	...
	$.each(rows, function(i, row) {
		$(row)
		.data("id", $(row).children('td:first-child').children('input').val());
		$(row)
		.data("type",
			$(row).children('td:first-child').children('input').attr('prim'));

		$(row)
		.data("name", $(row).children('td:first-child').children('a').text());
