		...
		scrollParent: function() {
			var position = this.css("position"),
				...
				scrollParent = this.parents().filter(function() {
					...
					return (/(auto|scroll)/).test(parent.css("overflow") + parent.css(
		...
		$.fn.position = function(options) {
			...
			return this.each(function() {
				var collisionPosition, using,
					...
					myOffset = getOffsets(offsets.my,
						elem.outerWidth(),
						elem.outerHeight());
			...
			flip: {
				...
				top: function(position, data) {
					...
					if (overTop < 0) {
						...
						if (( position.top + myOffset + atOffset + offset) > overTop && ( newOverBottom < 0 || newOverBottom < abs(
								overTop) )) {
							position.top += myOffset + atOffset + offset;
						}
					} else if (overBottom > 0) {
						newOverTop = position.top - data.collisionPosition.marginTop + myOffset + atOffset + offset - offsetTop;
						if (( position.top + myOffset + atOffset + offset) > overBottom && ( newOverTop > 0 || abs(
 ...
 */
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_create: function() {
			...
			if (this.options.helper === "original" && !(/^(?:r|a|f)/).test(this.element.css(
					"position"))) {
		...
		_createHelper: function(event) {
			...
			var o = this.options,
				helper = $.isFunction(o.helper) ? $(o.helper.apply(this.element[0],
					[event])) : (o.helper === "clone" ? this.element.clone()
		...
		_setContainment: function() {
			...
			this.containment = [
				...
				( parseInt(c.css("borderTopWidth"),
					10) || 0 ) + ( parseInt(c.css("paddingTop"), 10) || 0 ),
				...
				( over ? Math.max(ce.scrollHeight,
					ce.offsetHeight) : ce.offsetHeight ) - ( parseInt(c.css(
					"borderBottomWidth"), 10) || 0 ) - ( parseInt(c.css(
					"paddingBottom"),
			...
			var min,
				...
				group = $.makeArray($(o.stack)).sort(function(a, b) {
					return (parseInt($(a).css("zIndex"),
						10) || 0) - (parseInt($(b).css("zIndex"), 10) || 0);
				});
		...
		return function(draggable, droppable, toleranceMode) {
			...
			switch (toleranceMode) {
				...
				case "pointer":
					...
					return isOverAxis(draggableTop,
						t,
			...
			$.each($.ui.ddmanager.droppables[draggable.options.scope] || [],
				function() {
					...
					var parentInstance, scope, parent,
						intersects = $.ui.intersect(draggable,
							this,
							this.options.tolerance),
			...
			for (i = 0; i < this._proportionallyResizeElements.length; i++) {
				...
				if (!this.borderDif) {
					...
					for (j = 0; j < borders.length; j++) {
						this.borderDif[j] = ( parseInt(borders[j],
							10) || 0 ) + ( parseInt(paddings[j], 10) || 0 );
					}
				}
	...
	$.ui.plugin.add("resizable", "animate", {
		...
		stop: function(event) {
			var that = $(this).resizable("instance"),
				...
				ista = pr.length && (/textarea/i).test(pr[0].nodeName),
				soffseth = ista && that._hasScroll(pr[0],
					"left") /* TODO - jump height */ ? 0 : that.sizeDiff.height,
				soffsetw = ista ? 0 : that.sizeDiff.width,
				style = {
	...
	var selectable = $.widget("ui.selectable", $.ui.mouse, {
		...
		_mouseStart: function(event) {
			...
			$(event.target).parents().addBack().each(function() {
				...
				if (selectee) {
					doSelect = (!event.metaKey && !event.ctrlKey) || !selectee.$element.hasClass(
						"ui-selected");
					selectee.$element
 ...
 */
	...
	var sortable = $.widget("ui.sortable", $.ui.mouse, {
		...
		serialize: function(o) {
			...
			$(items).each(function() {
				var res = ($(o.item || this)
				.attr(o.attribute || "id") || "").match(o.expression || (/(.+)[\-=_](.+)/));
				if (res) {
						...
						if (inst && inst !== this && !inst.options.disabled) {
							queries.push([
								$.isFunction(inst.options.items) ? inst.options.items.call(
									inst.element) : $(inst.options.items,
									inst.element)
			...
			if (!o.placeholder || o.placeholder.constructor === String) {
				...
				o.placeholder = {
					...
					update: function(container, p) {
						...
						if (!p.height()) {
							p.height(that.currentItem.innerHeight() - parseInt(that.currentItem.css(
								"paddingTop") || 0, 10) - parseInt(that.currentItem.css(
								"paddingBottom") || 0, 10));
						}
						if (!p.width()) {
							p.width(that.currentItem.innerWidth() - parseInt(that.currentItem.css(
			...
			if (!(/^(document|window|parent)$/).test(o.containment)) {
				...
				this.containment = [
					...
					co.left + (over ? Math.max(ce.scrollWidth,
						ce.offsetWidth) : ce.offsetWidth) - (parseInt($(ce)
   ...
   allowing multiple different settings on the same page. */
	 ...
	 */
		...
		_doKeyPress: function(event) {
			...
			if ($.datepicker._get(inst, "constrainInput")) {
				...
				return event.ctrlKey || event.metaKey || (chr < " " || !chars || chars.indexOf(
					chr) > -1);
			}
		},
	 ...
	 */
		...
		_updateDatepicker: function(inst) {
			...
			if (inst === $.datepicker._curInst && $.datepicker._datepickerShowing && $.datepicker._shouldFocusInput(
					inst)) {
				inst.input.focus();
			}
		...
		_findPos: function(obj) {
			...
			while (obj && (obj.type === "hidden" || obj.nodeType !== 1 || $.expr.filters.hidden(
				obj))) {
	 ...
	 */
		...
		_determineDate: function(inst, date, defaultDate) {
			var offsetNumeric = function(offset) {
				...
				newDate = (date == null || date === "" ? defaultDate : (typeof date === "string" ? offsetString(
					date) :
				                                                        (typeof date === "number" ? (isNaN(
					                                                        date) ? defaultDate : offsetNumeric(
					                                                        date)) : new Date(
					                                                        date.getTime()))));
					...
					for (dRow = 0; dRow < numRows; dRow++) { // create date picker rows
						...
						for (dow = 0; dow < 7; dow++) { // create date picker days
							...
							tbody += "<td class='" +
								...
								((!otherMonth || showOtherMonths) && daySettings[2] ? " title='" + daySettings[2].replace(
									/'/g,
									"&#39;") + "'" : "") + // cell title
			...
			each(hooks, function(i, hook) {
				jQuery.cssHooks[hook] = {
					set: function(elem, value) {
						...
						if (value !== "transparent" && ( jQuery.type(value) !== "string" || ( parsed = stringParse(
								value) ) )) {
...
// based on easing equations from Robert Penner (http://www.robertpenner.com/easing)
		...
		$.extend(baseEasings, {
			...
			Bounce: function(p) {
				...
				return 1 / Math.pow(4, 3 - bounce) - 7.5625 * Math.pow(( pow2 * 3 - 2 ) / 22 - p,
					2);
			}
		});
